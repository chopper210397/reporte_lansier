#install.packages("RODBC")  #paquete necesario para conectarse al sql server desde R
#install.packages("writexl") #paquete utilizado para exportar un data frame hacia excel
...
#################################################################################################
#################################################################################################
## ESTE CODIGO GENERA EL EXCEL DATA PRESENTACION EXTRAYENDO LA DATA DIRECTAMENTE DEL SQL       ##
## DANDOLE EL FORMATO Y EXPORTANDOLO A EXCEL, ASI LA SECUENCIA SERIA, CORRER ESTE PROGRAMA     ##
## Y LUEGO CORRER AUTOMATICAMENTE EL RMARKDOWN Y EL PROGRAMA QUE ENVIA CORREO A LOS VENDEDORES ##
#################################################################################################
#################################################################################################
#################################################################################################
#################################################################################################
...
#####################################################################################
# CUANDO HE INTENTADO HACER LA CONEXION SQL SERVER DESDE OTRA MAQUINA NO ME PERMITE #
# MAS SEGURO PORQUE NO TENGO EL VPN ACTIVADO EN ESA MAQUINA                         #
#####################################################################################

# el nombre de la conexion es SQLuis
# con el paso de abajo logre conectarme a la base de datos
# y ya puedo traer toda la data del sql de lansier
#rm(sqlcomercial)
...
#con sql query realizamos sentencias sql y traemos de la base de datos
#expertvision<-sqlQuery(sqlcomercial,"select * from Venta_Lansier where ruc='20605547657'")

#cargando las fechas de carga de las distrbuidoras
...
############ HALLANDO LOS VALORES PARA TODAS LAS CATEGORIAS DE UNA ZONA ####################
...
azullimaeste<-azullimaeste %>%
  add_row(soles=sum(azullimaeste$soles),categorizacion="TOTALES",ppsol=sum(azullimaeste$ppsol))
...
azullimanorte<-azullimanorte %>%
  add_row(soles=sum(azullimanorte$soles),categorizacion="TOTALES",ppsol=sum(azullimanorte$ppsol))
...
azullimaoeste<-azullimaoeste %>%
  add_row(soles=sum(azullimaoeste$soles),categorizacion="TOTALES",ppsol=sum(azullimaoeste$ppsol))
...
azullimasur<-azullimasur %>%
  add_row(soles=sum(azullimasur$soles),categorizacion="TOTALES",ppsol=sum(azullimasur$ppsol))
...
rojolimacallao<-rojolimacallao %>%
  add_row(soles=sum(rojolimacallao$soles),categorizacion="TOTALES",ppsol=sum(rojolimacallao$ppsol))
...
rojolimaeste<-rojolimaeste %>%
  add_row(soles=sum(rojolimaeste$soles),categorizacion="TOTALES",ppsol=sum(rojolimaeste$ppsol))
...
rojolimanorte<-rojolimanorte %>%
  add_row(soles=sum(rojolimanorte$soles),categorizacion="TOTALES",ppsol=sum(rojolimanorte$ppsol))
...
rojolimaoeste<-rojolimaoeste %>%
  add_row(soles=sum(rojolimaoeste$soles),categorizacion="TOTALES",ppsol=sum(rojolimaoeste$ppsol))
...
rojolimasur<-rojolimasur %>%
  add_row(soles=sum(rojolimasur$soles),categorizacion="TOTALES",ppsol=sum(rojolimasur$ppsol))
...
nortegrandea<-nortegrandea %>%
  add_row(soles=sum(nortegrandea$soles),categorizacion="TOTALES",ppsol=sum(nortegrandea$ppsol))
...
nortegrandeb<-nortegrandeb %>%
  add_row(soles=sum(nortegrandeb$soles),categorizacion="TOTALES",ppsol=sum(nortegrandeb$ppsol))
...
nortemedio<-nortemedio %>%
  add_row(soles=sum(nortemedio$soles),categorizacion="TOTALES",ppsol=sum(nortemedio$ppsol))
...
surgrande<-surgrande %>%
  add_row(soles=sum(surgrande$soles),categorizacion="TOTALES",ppsol=sum(surgrande$ppsol))
...
suroriente<-suroriente %>%
  add_row(soles=sum(suroriente$soles),categorizacion="TOTALES",ppsol=sum(suroriente$ppsol))
...
write_xlsx(dffinal,"C:\\Users\\LBarrios\\Documents\\datapresentacion.xlsx")
